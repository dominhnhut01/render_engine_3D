...
double normalizeRadians(double rads) {
	/* CSE 386 - todo  */
	return glm::mod(rads, 2*glm::pi<double>());
}
...
double rad2deg(double rads) {
	/* CSE 386 - todo  */
	return rads / (2 * glm::pi<double>()) * 360.0;
}
...
double deg2rad(double degs) {
	/* CSE 386 - todo  */
	return degs / 360 * (2 * glm::pi<double>());
}
...
double distanceFromOrigin(double x, double y) {
	/* CSE 386 - todo  */
	return glm::pow((glm::pow(x,2) + glm::pow(y,2)), 0.5);
}
...
double distanceBetween(double x1, double y1, double x2, double y2) {
	/* CSE 386 - todo  */
	return glm::pow((glm::pow(x1-x2, 2) + glm::pow(y1-y2, 2)), 0.5);;
}
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
	/* CSE 386 - todo  */
	dvec2 point(0.0,0.0);
	point.x = center.x + glm::cos(angleRads) * R;
	point.y = center.y + glm::sin(angleRads) * R;
	return point;
}
...
double directionInRadians(double x1, double y1, double x2, double y2) {
	...
	return directionInRadians(refPt, targetPt);
}
...
bool isOrthogonal(const dvec3& a, const dvec3& b) {
	/* CSE 386 - todo  */
	if (glm::length(a) == 0 || glm::length(b) == 0)	return false;
...
bool formAcuteAngle(const dvec3& a, const dvec3& b) {
	/* CSE 386 - todo  */
	return glm::length (a + b) > glm::length(a-b);
}
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	/* CSE 386 - todo  */
	dvec3 vec1 = pointingVector(pt1, pt2);
	dvec3 vec2 = pointingVector(pt1, pt3);
	return areaOfParallelogram(vec1, vec2) / 2.0;
}
...
dvec3 pointingVector(const dvec3& pt1, const dvec3& pt2) {
	/* CSE 386 - todo  */
	dvec3 vec(pt2.x - pt1.x, pt2.y - pt1.y, pt2.z - pt1.z);
	return vec;
}
...
double assertZero(double zero) {
	if (approximatelyZero(zero))
		return 0;
	return zero;
}
...
vector<double> quadratic(double A, double B, double C) {
	...
	double delta = glm::pow(B, 2.0) - 4.0 * A * C;
	if (delta < 0) return result;
	double root1 = assertZero((-B - glm::pow(delta, 0.5)) / (2 * A));
	double root2 = assertZero((-B + glm::pow(delta, 0.5)) / (2 * A));
...
int quadratic(double a, double b, double c, double roots[2]) {
	...
	vector<double> roots_vec = quadratic(a, b, c);
	rootCnt = static_cast<int>(roots_vec.size());
